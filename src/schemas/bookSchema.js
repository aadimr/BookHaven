...
export const bestSellingBookSchema = Yup.object({
    ...
    book_Name: Yup.string().min(2, "Book name must be at least 2 characters").matches(/^(?! )[A-Za-z0-9]+(?: [A-Za-z0-9]+)*(?<! )$/, "Not valid").required("Please enter the book name"),
    ...
    review: Yup.string().matches(/^\d+(\.\d+)?$/, "Review must be number").test('max-value', 'Review must be less than or equal to 5', value => value <= 5).required("Please enter the review"),
})
